package sun.com.didi.chat.singlechat;
...
import sun.com.didi.entity.UserMap;

public class SingleChatHandler extends SimpleChannelInboundHandler {
    ...
    protected void channelRead0(ChannelHandlerContext ctx, Object msg) throws Exception {
        if (msg instanceof FullHttpRequest) { //是http请求
            ...
            if (message.indexOf("search") != -1) {
                Channel ch = this.usermap.getUser(message.split(":")[1].trim());
                if (ch != null) { //此用户存在
                    ctx.channel().writeAndFlush(new TextWebSocketFrame("search:"+message.split(":")[1].trim()+":已找到"));
                } else { // 此用户不存在
                    ctx.channel().writeAndFlush(new TextWebSocketFrame("search:"+message.split(":")[1].trim()+":未找到"));
                }

            }

            //发送消息给指定的用户    消息模型  me:to:[msg]
            if (message.split(":").length == 3) {  //判断是单聊消息
                this.usermap.getUser(message.split(":")[1].trim()).writeAndFlush(new TextWebSocketFrame(message));
            }
